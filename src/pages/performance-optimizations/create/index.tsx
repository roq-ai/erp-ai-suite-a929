...
import { createPerformanceOptimization } from 'apiSdk/performance-optimizations';
...
function PerformanceOptimizationCreatePage() {
  ...
  const handleSubmit = async (values: PerformanceOptimizationInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await createPerformanceOptimization(values);
      resetForm();
  ...
  const formik = useFormik<PerformanceOptimizationInterface>({
    initialValues: {
      optimization_date: new Date(new Date().toDateString()),
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Create Performance Optimization',
              isCurrent: true,
            },
          ]}
        />
...
)(PerformanceOptimizationCreatePage);
