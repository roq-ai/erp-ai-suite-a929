...
import { getDataPrivacyAssessments } from 'apiSdk/data-privacy-assessments';
import { DataPrivacyAssessmentInterface } from 'interfaces/data-privacy-assessment';
import { getSecurityAssessments } from 'apiSdk/security-assessments';
import { SecurityAssessmentInterface } from 'interfaces/security-assessment';
import { getLogAnalyses } from 'apiSdk/log-analyses';
import { LogAnalysisInterface } from 'interfaces/log-analysis';

import { PerformanceOptimizationListPage as ShowCaseList } from 'pages/performance-optimizations';

function DashboardPage() {
  const { session } = useSession();

  const dataPrivacyAssessmentStat = useSWR<PaginatedInterface<DataPrivacyAssessmentInterface>>(
    () => `/data-privacy-assessments`,
    () => getDataPrivacyAssessments({ limit: 1, offset: 0 }),
  );

  const securityAssessmentStat = useSWR<PaginatedInterface<SecurityAssessmentInterface>>(
    () => `/security-assessments`,
    () => getSecurityAssessments({ limit: 1, offset: 0 }),
  );

  const logAnalysisStat = useSWR<PaginatedInterface<LogAnalysisInterface>>(
    () => `/log-analyses`,
    () => getLogAnalyses({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Data Privacy Assessments',
      result: dataPrivacyAssessmentStat?.data?.totalCount,
      isLoading: dataPrivacyAssessmentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Security Assessments',
      result: securityAssessmentStat?.data?.totalCount,
      isLoading: securityAssessmentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Log Analyses',
      result: logAnalysisStat?.data?.totalCount,
      isLoading: logAnalysisStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`performance-optimizations`} passHref legacyBehavior>
                  <Button
                  ...
                  >
                    See all Performance Optimizations
                  </Button>
                </NextLink>
              </Flex>
